...
def start_create_account(bot, call):
    ...
    # إذا لم يكن لديه حساب مسبق، بدء إنشاء الحساب
    bot.edit_message_text(
        chat_id=chat_id,
        message_id=message_id,
...
def process_username_step(bot, message, telegram_id):
    ...
    try:
        ...
        # شريط التقدم عند التحقق من الاسم
        progress_message = bot.send_message(message.chat.id, "🔄 جاري التحقق: 0%")
        for i in range(0, 101, 20):
            time.sleep(0.3)
            try:
                bot.edit_message_text(chat_id=message.chat.id, message_id=progress_message.message_id,
                                      text=f"🔄 جاري التحقق: {i}%")
            except:
                pass
        # حذف الشريط بعد 100٪
...
def process_password_step(bot, message, telegram_id, username):
    ...
    try:
        ...
        progress_message = bot.send_message(message.chat.id, "🔄 جاري إنشاء الحساب: 0%")
        for i in range(0, 101, 25):
            time.sleep(0.5)
            try:
                bot.edit_message_text(chat_id=message.chat.id, message_id=progress_message.message_id,
                                      text=f"🔄 جاري إنشاء الحساب: {i}%")
            except:
                pass
        bot.delete_message(message.chat.id, progress_message.message_id)
