...
def make_username_available(base_username):
    ...
    while api.check_player_exists(username) and attempt < 10:
...
def process_username(message, bot):
    ...
    if user_id not in create_sessions:
        bot.send_message(message.chat.id, "❌ انتهت الجلسة، اضغط إنشاء حساب من جديد")
        return
    ...
    if not base_username.isascii() or len(base_username) < 4:
        ...
        return bot.register_next_step_handler(msg, process_username, bot)
...
def process_password(message, bot):
    ...
    if len(password) < 8:
        ...
        return bot.register_next_step_handler(msg, process_password, bot)
